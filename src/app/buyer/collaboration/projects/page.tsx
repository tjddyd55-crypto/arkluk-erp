  ...
  return (
    <div className="p-6">
      ...
      {!loading && !error && rows.length === 0 ? (
        <p className="text-sm text-slate-500">등록된 협업 프로젝트가 없습니다.</p>
      ) : null}
      <ul className="mt-4 space-y-2">
        {rows.map((r) => (
          <li key={r.id} className="rounded border border-slate-200 bg-white p-3">
            <Link href={`/buyer/collaboration/projects/${r.id}`} className="font-medium text-slate-900 hover:underline">
              {r.title}
            </Link>
