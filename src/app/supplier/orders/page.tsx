          ...
          <table className="min-w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#111318]">
                ...
                <th className="border border-[#2d333d] px-2 py-1 text-left">ETA</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.order_id}>
                  <td className="border border-[#2d333d] px-2 py-1">
                    <a className="text-blue-400 underline" href={`/supplier/orders/${row.order_id}`}>
                      {row.order.order_no}
                    </a>
                  </td>
                  <td className="border border-[#2d333d] px-2 py-1">{row.order.buyer.name}</td>
                  <td className="border border-[#2d333d] px-2 py-1">{row.order.country.country_name}</td>
                  <td className="border border-[#2d333d] px-2 py-1">
                    {row.sent_at ? new Date(row.sent_at).toLocaleString() : "-"}
                  </td>
                  <td className="border border-[#2d333d] px-2 py-1">{getStatusLabel(row)}</td>
                  <td className="border border-[#2d333d] px-2 py-1">
