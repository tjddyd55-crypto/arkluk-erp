...
import { markShipmentShipped } from "@/server/services/shipment-service";
...
export async function POST(
  ...
  try {
    ...
    const updated = await markShipmentShipped(parsedShipmentId, user.id);
    await createAuditLog({
      actorId: user.id,
      actionType: "SUPPLIER_MARK_SHIPMENT_SHIPPED",
      targetType: "SHIPMENT",
