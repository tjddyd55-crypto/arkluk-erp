...
import { shipmentItemAddSchema } from "@/lib/schemas";
import { createAuditLog } from "@/server/services/audit-log";
import { addShipmentItem } from "@/server/services/shipment-service";
...
export async function POST(
  ...
  try {
    ...
    const parsed = shipmentItemAddSchema.safeParse(await request.json());
    if (!parsed.success) {
      throw new HttpError(400, "출고 품목 추가 요청 형식이 올바르지 않습니다.");
    }

    const shipmentItem = await addShipmentItem(
      parsedShipmentId,
    ...
    await createAuditLog({
      actorId: user.id,
      actionType: "SUPPLIER_ADD_SHIPMENT_ITEM",
      targetType: "SHIPMENT",
    ...
    return ok(shipmentItem, { status: 201 });
  } catch (error) {
