...
export function BuyerProjectsList() {
  ...
  return (
    <section className="space-y-3 rounded border border-[#2d333d] bg-[#1a1d23] p-4">
      ...
      {loading ? <p className="text-sm text-gray-400">프로젝트 목록을 불러오는 중...</p> : null}
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead>
              <tr className="bg-[#111318]">
                ...
                <th className="border border-[#2d333d] px-2 py-1 text-left">최근 견적</th>
                <th className="border border-[#2d333d] px-2 py-1 text-left">최근 주문</th>
              ...
              {projects.map((project) => (
                <tr key={project.id}>
                  <td className="border border-[#2d333d] px-2 py-1">
                    <a className="text-blue-400 underline" href={`/buyer/projects/${project.id}`}>
                  ...
                  <td className="border border-[#2d333d] px-2 py-1">{project._count.quotes}</td>
                  <td className="border border-[#2d333d] px-2 py-1">{project._count.orders}</td>
                  <td className="border border-[#2d333d] px-2 py-1">
