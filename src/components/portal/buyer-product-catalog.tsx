...
export function BuyerProductCatalog() {
  ...
  return (
    <section className="space-y-4 rounded-lg border border-[#2d333d] bg-[#1a1d23] p-4">
      <div className="grid gap-3 md:grid-cols-[240px_1fr]">
        <label className="text-sm text-gray-400">
          {t("supplier")}
          <select
            className="mt-1 w-full rounded border border-[#2d333d] px-2 py-2 text-sm"
            value={supplierId ?? ""}
            onChange={(event) =>
        ...
        <label className="text-sm text-gray-400">
          ...
          <input
            ...
            className="mt-1 w-full rounded border border-[#2d333d] px-3 py-2 text-sm"
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse border border-[#2d333d] text-sm">
          <thead className="bg-[#111318]">
            <tr>
              ...
              <th className="border border-[#2d333d] px-2 py-1 text-right">Price</th>
            </tr>
          </thead>
          <tbody>
            {products.length === 0 ? (
              <tr>
                <td
                  colSpan={6}
                  className="border border-[#2d333d] px-2 py-6 text-center text-gray-400"
                >
                  {t("no_data")}
                </td>
              </tr>
              ...
              products.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="border border-[#2d333d] px-2 py-1 text-right">{product.price}</td>
                </tr>
              ))
            )}
          </tbody>
