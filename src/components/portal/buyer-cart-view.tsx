  ...
  return (
    <div className="space-y-4">
      ...
      {data.grouped.length === 0 ? (
        <p className="text-sm text-slate-500">{t("buyer_cart_empty")}</p>
      ) : (
        <>
          {data.grouped.map((g) => (
            <section key={g.supplierId} className="rounded border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-semibold text-slate-900">{g.supplierLabel}</h3>
              <ul className="mt-2 space-y-3">
                {g.items.map((it) => (
                  <li
                    key={it.id}
                    className="flex flex-wrap items-end justify-between gap-2 border-b border-slate-100 pb-3 last:border-0"
                  >
                    <div>
                      <p className="font-medium text-slate-800">{it.displayName}</p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-200 bg-slate-50 p-4">
